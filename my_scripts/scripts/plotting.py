...
def plotting_class(prob_cube,saveflag,saveroot,img_num):
    
    timesteps = 100
    
    class0_prob_mean = np.mean(prob_cube[1].numpy(), axis=0)
    class1_prob_mean = np.mean(prob_cube[0].numpy(), axis=0)

    class0_prob_std = np.std(prob_cube[1].numpy(),axis=0)/np.sqrt(img_num)
    class1_prob_std = np.std(prob_cube[0].numpy(),axis=0)/np.sqrt(img_num)
    ...
    plt.subplot(1,4,1)
    xdata = np.arange(timesteps)
    plt.errorbar(xdata,class0_prob_mean,label='Illusion',yerr=class0_prob_std,c='g',ecolor='g',errorevery=10)
    plt.errorbar(xdata,class1_prob_mean,label='Cube', yerr=class1_prob_std,c='k',ecolor='k',errorevery=10)
    plt.legend()
