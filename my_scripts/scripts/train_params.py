...
from mydata import MyDataset1
from architecture import PredictiveCoder, PC_Params
from train_process import pretrain_process, finetune_process
...
def finetuning(device,alpha,train_root,valid_root,root_path,trainbatch,valbatch,timesteps=10,net1number = 1,net2number=11,max_epoch = 25,CheckpointFlag1 = False,checkpoint1 = None,checkpoint2=None):
    ...
    train_loader    = torch.utils.data.DataLoader(train_sets, batch_size=trainbatch, shuffle=True,  num_workers=12, drop_last=False)
    val_loader      = torch.utils.data.DataLoader(val_sets,   batch_size=valbatch,   shuffle=False, num_workers=12, drop_last=False)
    
    # finetune the network with shapes dataset
    if CheckpointFlag1:
        finetune_process(pc_params, net,train_loader,trainset_size,val_loader,valset_size,device, net2number,root_path, max_epoch=max_epoch, CheckpointFlag1=CheckpointFlag1,checkpoint=checkpoint1)
    else:
