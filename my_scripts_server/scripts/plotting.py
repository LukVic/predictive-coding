...
def plotting_class(prob_square,saveflag,saveroot,img_num):
    ...
    prob_square0 = prob_square[0].to('cpu')
    prob_square1 = prob_square[1].to('cpu')
    prob_square2 = prob_square[2].to('cpu')
    prob_square3 = prob_square[3].to('cpu')
    class0_prob_mean = np.mean(prob_square0.numpy(), axis=0)
    class1_prob_mean = np.mean(prob_square1.numpy(), axis=0)
    class2_prob_mean = np.mean(prob_square2.numpy(), axis=0)
    class3_prob_mean = np.mean(prob_square3.numpy(), axis=0)

    class0_prob_std = np.std(prob_square0.numpy(),axis=0)/np.sqrt(img_num)
    class1_prob_std = np.std(prob_square1.numpy(),axis=0)/np.sqrt(img_num)
    class2_prob_std = np.std(prob_square2.numpy(),axis=0)/np.sqrt(img_num)
    class3_prob_std = np.std(prob_square3.numpy(),axis=0)/np.sqrt(img_num)
