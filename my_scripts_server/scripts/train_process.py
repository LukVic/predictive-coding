...
from loss import ae_loss,cc_loss

def pretrain_process(pc_params, net,train_loader,val_loader,device, netnumber, max_epoch=200, CheckpointFlag1=False,checkpoint=None ):
    ...
    for epoch in torch.arange(start_epoch,max_epoch):   # loop over the dataset multiple times
        ...
        print("Epoch  mod: {0}".format(epoch % 10))
        if epoch % 10 == 1:
        ...
        if epoch % 10 == 1: # 10 == 9
            ...
            PRE = "/home.nfs/vicenluk/KSY/Illusory-Contour-Predictive-Networks/"
            PATH = PRE + f'train/train1_t{timesteps}_net{netnumber}_ep{epoch}.pth'     
...
def finetune_process(pc_params, net,train_loader,trainset_size,val_loader,valset_size,device, netnumber, root_path,max_epoch=30, CheckpointFlag1=False,checkpoint=None ):
    ...
    for epoch in torch.arange(start_epoch,max_epoch):
        ...
        running_detail_acc =  [running_detail_acc[c].cpu()/trainset_size for c in range(timesteps)]
        ...
        running_detail_acc =  [running_detail_acc[c].cpu()/valset_size for c in range(timesteps)]        
        ...
        print('toto je root_path: {0}'.format(root_path))
        # save model and variables
        if epoch % 5 == 4:
            PATH =  root_path + f'/train2_t{timesteps}_net{netnumber}_ep{epoch}.pth'
            print('toto je root_path: {0}'.format(PATH))
            torch.save({'model_state_dict':net.state_dict(),
                        'optimizer_state_dict':optimizer.state_dict(),
                        'start_epoch':epoch+1,
                        'TV_loss':TV_loss,
